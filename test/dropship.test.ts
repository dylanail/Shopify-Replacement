...
test('every advertorial and pdp format writes a complete page in every tone', () => {
  ...
  for (const type of ['rating-line', 'steps', 'citations', 'expert-quote', 'ingredients', 'buy-box', 'disclaimer']) assert.ok(scienceTypes.includes(type), `science page has ${type}`)
  assert.ok(scienceTypes.indexOf('citations') < scienceTypes.indexOf('buy-box'), 'the research comes before the offer')
  ...
  assert.ok(salesTypes.indexOf('buy-box') < salesTypes.indexOf('timeline'), 'the buy box is on top, the argument below')
